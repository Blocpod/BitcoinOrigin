...
export function createConsistencyProof(oldHashes, newHashes) {
  if (!Array.isArray(oldHashes) || !Array.isArray(newHashes)) throw new TypeError('hash lists are required');
  if (oldHashes.length > newHashes.length) throw new Error('old tree cannot be larger than new tree');
  for (let index = 0; index < oldHashes.length; index += 1) {
    if (oldHashes[index] !== newHashes[index]) throw new Error(`new tree is not an append-only extension at index ${index}`);
  }
  return {
    version: 1,
    oldSize: oldHashes.length,
    newSize: newHashes.length,
    oldRoot: merkleRoot(oldHashes),
    newRoot: merkleRoot(newHashes),
    oldLeaves: [...oldHashes],
    appendedLeaves: newHashes.slice(oldHashes.length)
  };
}

export function verifyConsistencyProof(proof) {
  if (!proof || !Array.isArray(proof.oldLeaves) || !Array.isArray(proof.appendedLeaves)) return false;
  if (proof.oldLeaves.length !== proof.oldSize) return false;
  if (proof.oldLeaves.length + proof.appendedLeaves.length !== proof.newSize) return false;
  if (merkleRoot(proof.oldLeaves) !== proof.oldRoot) return false;
  return merkleRoot([...proof.oldLeaves, ...proof.appendedLeaves]) === proof.newRoot;
}

export function compareCheckpointViews(left, right) {
  if (!left || !right) return { conflict: true, reason: 'checkpoint missing' };
  if (left.operator !== right.operator) return { conflict: false, reason: 'different operators' };
  if (left.size === right.size && left.root !== right.root) return { conflict: true, reason: 'same operator and tree size published different roots' };
  if (left.size === right.size && left.root === right.root) return { conflict: false, reason: 'matching checkpoint views' };
  return { conflict: false, reason: 'different tree sizes require a consistency proof' };
}
